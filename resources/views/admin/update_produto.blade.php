...
<div class="d-flex mx-auto flex-column">
    ...
    @foreach ($updateProduto as $produto)
        <form action="{{ route('admin.update', $produto->id_produto) }}" method="post"
            class="d-flex flex-column w-50 mx-auto" enctype="multipart/form-data">
            @csrf
            ...
            @foreach ($imgs as $img)
                <img src="{{ asset('storage/images/produtos/' . $img) }}" class="img-fluid mt-4" width="200px"
                    alt="IMAGEM TESTE">
            @endforeach
